...
router.post('/app/adddiseasePage', (req, res) => {
    ...
    if (_.isString(diseaseName) && !_.isNaN(diseaseScore)) {
        ...
        disease.save().then((disease) => {
            ...
            res.status(200).redirect('/app/adddiseasePage');
        }).catch((err) => {
            console.log(err);
            res.status(400).redirect('/app/adddiseasePage');
        });
    } else {
        res.status(400).redirect('/app/adddiseasePage',{messages: req.flash('success_msg', 'Succesful test') });
    }
});
